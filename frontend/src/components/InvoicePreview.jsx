...
function resolveImageUrl(url) {
  ...
  // If it contains localhost or 127.0.0.1, rewrite to API_BASE.
...
export default function InvoicePreview() {
  ...
  // const [invoiceError, setInvoiceError] = useState(null); // Removed unused
  ...
  useEffect(() => {
    ...
    async function fetchInvoice() {
      ...
      // setInvoiceError(null); // Removed unused
      try {
        ...
        if (mounted) {
          ...
          // else setInvoiceError("Invoice not found"); // Removed unused
  ...
  /* -------- set document.title to invoice filename -------- */
  useEffect(() => {
    if (!invoice) return;
    const invoiceNumber =
  ...
  /* -------- print handler that opens browser print dialog -------- */
  const handlePrint = useCallback(() => {
    ...
    // Set title to invoice number for print dialog filename
    document.title = safe;
