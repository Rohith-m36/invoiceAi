...
/* helpers to format icons (unchanged) */
...
export default function Dashboard() {
  ...
  // optional: store business profile for branding/defaults
  const [businessProfile, setBusinessProfile] = useState(null);

  // fetch invoices from backend and map to client UI shape (preserve currency)
  ...
  useEffect(() => {
    ...
    // optional: subscribe to storage events in case other parts of app update localStorage
  ...
  // Hard-coded conversion rates (no external API)
  // You asked to use a fixed value — update here if you want a different USD→INR conversion.
  const HARD_RATES = {
    USD_TO_INR: 83, // fixed rate: 1 USD = 83 INR
  };

  // convert arbitrary currency amount -> INR using hard-coded mapping
  function convertToINR(amount = 0, currency = "INR") {
    ...
    // fallback for unknown currencies: return raw amount (no conversion)
    return n;
  }

  // Enhanced KPI computation with percentages (convert each invoice to INR before summing)
  const kpis = useMemo(() => {
  ...
  }, [storedInvoices]); // removed rates dependency (we're using a hard-coded value)

  // recent invoices (sorted by issueDate desc)
  ...
  // Helper function to get client initial safely
  const getClientInitial = (inv) => {
    const clientName = getClientName(inv);
    return clientName ? clientName.charAt(0).toUpperCase() : "C";
  };

  // OPEN behavior — navigate to invoice preview (keeps previous navigation target)
  ...
  return (
    <div className={dashboardStyles.pageContainer}>
      ...
      {/* Enhanced KPI row with progress indicators */}
              ...
              <table className={dashboardStyles.table}>
                <thead>
                  <tr className={dashboardStyles.tableHead}>
                    ...
                    <th className={dashboardStyles.tableHeaderCellRight}>
                      Actions
                    </th>
                  </tr>
                </thead>
